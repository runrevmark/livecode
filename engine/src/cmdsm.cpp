...
Parse_stat MCSetOp::parse(MCScriptPoint &sp)
{
    ...
    {
        if (op == kOpIntersect)
            op = kOpRecursiveIntersect;
        else if (op == kOpUnion)
            op = kOpRecursiveUnion;
        else
        {
...
void MCSetOp::exec_ctxt(MCExecContext &ctxt)
{
	...
	switch(op)
    {
        case kOpIntersect:
            ...
            break;
        case kOpRecursiveIntersect:
            MCArraysExecRecursiveIntersect(ctxt, *t_dst, *t_src, &t_dst_value);
            break;
        case kOpUnion:
            MCArraysExecUnion(ctxt, *t_dst, *t_src, &t_dst_value);
            break;
        case kOpRecursiveUnion:
            MCArraysExecRecursiveUnion(ctxt, *t_dst, *t_src, &t_dst_value);
            break;
        case kOpDifference:
