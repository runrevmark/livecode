...
static bool MCGContextStateCreate(MCGContextStateRef& r_state)
{
	...
	if (t_success)
	{
		...
		t_state -> fill_paint = nullptr; //MCGRetain(kMCBlackSolidColor);
...
MCGContextSetGradientPaint(MCGContextRef self, MCGGradientFunction p_function, const MCGFloat* p_stops, const MCGColor* p_colors, uindex_t p_ramp_length, bool p_mirror, bool p_wrap, uint32_t p_repeats, MCGAffineTransform p_transform, MCGImageFilter p_filter, MCGPaintRef& x_target)
{
    ...
    switch(p_function)
    {
        ...
        case kMCGLegacyGradientDiamond:
            MCGContextSetNonePaint(self, x_target);
            break;
        case kMCGGradientFunctionLinear:
            if (!MCGLinearGradient::Create({0, 0}, {t_gradient_scale, 0}, t_ramp, t_spread, p_transform, t_gradient))
            {
                self->is_valid = false;
                return;
            }
        ...
        case kMCGGradientFunctionRadial:
            if (!MCGRadialGradient::Create({0, 0}, t_gradient_scale, t_ramp, t_spread, p_transform, t_gradient))
            {
                self->is_valid = false;
                return;
            }
        ...
        case kMCGGradientFunctionSweep:
            if (!MCGSweepGradient::Create({0, 0}, t_ramp, t_spread, p_transform, t_gradient))
            {
                self->is_valid = false;
                return;
            }
